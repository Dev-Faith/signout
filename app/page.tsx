...
import { Shirt, ChevronRight, Check } from "lucide-react";
...
export default function Home() {
  ...
  return (
    <main className="w-full min-h-screen flex flex-col items-center justify-center bg-slate-50 relative overflow-hidden py-10">
      ...
      {!needsOnboarding ? (
        ...
        <div className="glass-panel p-8 max-w-lg w-full shadow-2xl border-white/50 relative">
          <h2 className="text-2xl font-bold text-slate-800 mb-6">Customize Your Shirt</h2>
          
          <div className="space-y-6">
            <div>
              <label className="block text-sm font-semibold text-slate-600 mb-2">Chest Text</label>
              <input 
                type="text" 
                value={customText}
                onChange={(e) => setCustomText(e.target.value)}
                maxLength={25}
                className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-white/50 focus:bg-white focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all font-medium"
                placeholder="e.g. Class of 2026"
              />
            </div>

            <div>
              <label className="block text-sm font-semibold text-slate-600 mb-3">Front Design</label>
              <div className="grid grid-cols-2 gap-3">
                {DESIGNS.map(d => (
                  <button
                    key={d.id}
                    onClick={() => setSelectedDesign(d.id)}
                    className={`relative p-4 rounded-xl border-2 transition-all flex flex-col items-center justify-center gap-2 ${selectedDesign === d.id ? 'border-primary bg-primary/5' : 'border-slate-100 bg-white hover:border-slate-200'}`}
                  >
                    <img src={`/${d.id}`} alt={d.name} className="w-12 h-12 object-contain mix-blend-multiply opacity-80" />
                    <span className={`text-xs font-semibold ${selectedDesign === d.id ? 'text-primary' : 'text-slate-500'}`}>{d.name}</span>
                    {selectedDesign === d.id && (
            ...
            <button 
              ...
              disabled={isSaving}
              className="w-full mt-2 bg-primary text-white font-bold py-4 px-8 rounded-xl shadow-lg hover:shadow-primary/30 hover:bg-primary/90 transition-all flex items-center justify-center gap-2 disabled:opacity-50"
            >
              {isSaving ? "Creating..." : "Create My Shirt"}
